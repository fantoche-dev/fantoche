...
issue labeled `good first issue`, or to open an issue describing what you'd like
to build.
...
4. **C++** — rendering backend work happens upstream in ThorVG first (see
   `docs/adr/0003`), then in our backend seam (from P4 onward).
...
- Every PR must keep CI green (Linux + macOS build/test, render smoke, golden
  frames, no-telemetry guard).
- Golden-frame changes must be regenerated on Linux (CI) and explained in the PR
  description.
- Architecture-level changes need an ADR in `docs/adr/` (copy the format of the
  existing ones) — propose it in an issue first.
