...
```tsx
import {Audio, Img, Video, makeScene2D} from '@fantoche-dev/2d';
import {all, chain, createRef, waitFor} from '@fantoche-dev/core';
...
<details>
  <summary>
    <code>npm init @fantoche-dev@latest</code> fails to execute.
  </summary>
...
```bash
npm exec @fantoche-dev/create@latest
```
