...
```tsx
import {Audio, Img, Video, makeScene2D} from '@fantoche/2d';
import {all, chain, createRef, waitFor} from '@fantoche/core';
...
<details>
  <summary>
    <code>npm init @fantoche@latest</code> fails to execute.
  </summary>
...
```bash
npm exec @fantoche/create@latest
```
