import type {FfmpegExporterOptions} from '@fantoche/core';
...
export function getParamDefaultsAndCheckValidity(settings: RenderSettings): {
  ...
  if (
    settings.projectSettings?.exporter?.name === '@fantoche/core/image-sequence'
  ) {
    throw Error(
  ...
  if (
    settings.projectSettings?.exporter?.name === '@fantoche/core/wasm' &&
    extension !== 'mp4'
  ) {
    throw Error(
  ...
  if (settings.projectSettings?.exporter?.name === '@fantoche/core/wasm') {
    return {
      ...defaultReturn,
