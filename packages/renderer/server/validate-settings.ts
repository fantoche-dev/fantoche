import type {FfmpegExporterOptions} from '@fantoche-dev/core';
...
export function getParamDefaultsAndCheckValidity(settings: RenderSettings): {
  ...
  if (
    settings.projectSettings?.exporter?.name === '@fantoche-dev/core/wasm' &&
    extension !== 'mp4'
  ) {
    throw Error(
  ...
  if (settings.projectSettings?.exporter?.name === '@fantoche-dev/core/wasm') {
    return {
      ...defaultReturn,
