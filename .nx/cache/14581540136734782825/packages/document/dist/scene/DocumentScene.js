...
export class DocumentScene extends AbstractScene {
    ...
    async applyState(localFrame) {
        ...
        this.execute(() => {
            ...
            // Force lazy computeds that need the scene context (SVG parsing, code
            // measurement, flex layout) to realize HERE, inside execute() — render()
            // runs without scene context and silently drops nodes whose computeds
            // first fire there.
            this.view.requestLayoutUpdate();
        });
