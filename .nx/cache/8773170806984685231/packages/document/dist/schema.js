...
export const idSchema = z
    .string()
    .regex(/^[A-Za-z_][A-Za-z0-9_-]*$/, 'ids start with a letter or _ and contain only letters, digits, _ and -');
...
export const timeRefSchema = z.union([
    ...
    z.string().regex(ANCHOR_RE, {
        message: 'not a valid anchor — expected "segment.start", "segment.end" or "segment.word:<word>", optionally followed by +/-<seconds>',
    }),
]);
...
const svgElement = z.strictObject({
    ...
    props: z
        ...
        message: 'svg elements need exactly one of svg (inline markup) or src (asset id)',
    }),
});
...
const tweenItem = z.strictObject({
    ...
    target: idSchema,
    tween: z
        .record(z.string().min(1), z.strictObject({ to: propValueSchema, from: propValueSchema.optional() }))
        .refine(record => Object.keys(record).length > 0, {
...
const blockItem = z.strictObject({
    ...
    block: z.strictObject({
        src: z
            .string()
            .regex(/^.+#[A-Za-z_$][A-Za-z0-9_$]*$/, 'block src must be "<module path>#<export name>"'),
        dur: positiveSeconds,
    }),
});
