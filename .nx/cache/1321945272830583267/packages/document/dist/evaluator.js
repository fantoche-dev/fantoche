import { Color, easeInBack, easeInBounce, easeInCirc, easeInCubic, easeInElastic, easeInExpo, easeInOutBack, easeInOutBounce, easeInOutCirc, easeInOutCubic, easeInOutElastic, easeInOutExpo, easeInOutQuad, easeInOutQuart, easeInOutQuint, easeInOutSine, easeInQuad, easeInQuart, easeInQuint, easeInSine, easeOutBack, easeOutBounce, easeOutCirc, easeOutCubic, easeOutElastic, easeOutExpo, easeOutQuad, easeOutQuart, easeOutQuint, easeOutSine, linear, map, } from '@fantoche-dev/core';
 ...
 */
export function lerpValue(from, to, value) {
    ...
    if (Array.isArray(from) && Array.isArray(to) && from.length === to.length) {
        return from.map((entry, i) => lerpValue(entry, to[i], value));
    }
    ...
    if (typeof from === 'string' && typeof to === 'string') {
        ...
        try {
            return Color.lerp(from, to, clamped).serialize();
        }
        catch {
...
function lastAtOrBefore(entries, frame, time) {
    let low = 0;
 ...
 */
export function evaluateFrame(ir, frame) {
    const props = new Map();
    ...
    for (const track of ir.codeTracks) {
        let codeState = track.initialCode;
        ...
        let selection = {
            ranges: cloneRanges(track.initialSelection ?? [
                [
                    [0, 0],
                    [Infinity, Infinity],
                ],
...
export function evaluate(ir, tSeconds) {
