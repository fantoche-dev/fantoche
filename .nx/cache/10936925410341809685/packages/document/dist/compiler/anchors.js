...
export function buildNarrationIndex(narration) {
...
export function resolveTimeRef(ref, index) {
    ...
    if (segment === undefined) {
        throw new AnchorError(`"${ref}" references unknown narration segment "${anchor.segment}"`);
    }
    let base;
    const warnings = [];
    if (anchor.kind === 'start') {
        ...
        if (segment.words === null) {
            throw new AnchorError(`"${ref}": segment "${anchor.segment}" has no word timings — ` +
        ...
        if (starts === undefined || starts.length === 0) {
            throw new AnchorError(`"${ref}": word "${anchor.word}" not found in segment "${anchor.segment}"`);
        }
        base = starts[0];
        if (starts.length > 1) {
            warnings.push(`"${ref}" is ambiguous — "${anchor.word}" occurs ${starts.length} ` +
                `times in "${anchor.segment}"; using the first occurrence`);
        }
    }
    if (anchor.kind === 'word' && anchor.offset !== 0) {
        warnings.push(`"${ref}": trailing ${anchor.offset > 0 ? '+' : ''}${anchor.offset} was ` +
